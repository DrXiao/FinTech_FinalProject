...
data_path = Path('test/')
train_dataset_file = data_path / 'train.csv'
test_dataset_file = data_path / 'test.csv'
...
def return_by_lump_sum(date, price):
  begin_date = date.min()
  end_date = date.max()
  begin_price = float(price[price['年月']==begin_date]['收盤價(元)_年'].unique())
  end_price = float(price[price['年月']==end_date]['收盤價(元)_年'].unique())
...
def return_by_dca(date, price):
  begin_date = date.min()
  end_date = date.max()
  avg_price = float(np.mean(price['收盤價(元)_年'].unique()))
  end_price = float(price[price['年月']==end_date]['收盤價(元)_年'].unique())
...
def select_portfolio(model, train_d, val_d):
  ...
  for stock in good_stocks:
    ...
    # print(first_year[first_year['簡稱']==stock]['本益比'].unique())
    cur_PER = float(first_year[first_year['簡稱']==stock]['本益比'].unique())
...
if __name__=="__main__":
  ...
  # Load the dataset
  train_d = StockDataset(train_dataset_file).filted_d
  test_d = StockDataset(test_dataset_file).filted_d
  ...
  print(f"Train data: {len(train_d)}, Test data: {len(test_d)}")

  # Train the model to select index and portfolio
  print("Training...")
  model = select_index(train_d, test_d)
  portfolio = select_portfolio(model, train_d, test_d)
  ...
  avg_dca_return, \
  avg_dca_yearly_mean_return = evaluate_portfolio(portfolio, test_d)

  print("Total Lump-Sum return:")
  print(f"\tTotal Return: {avg_lumpsum_return:.2f}%")
  print(f"\tYearly Mean Return: {avg_lumpsum_yearly_mean_return:.2f}%")
  print(f"Total DCA return:")
